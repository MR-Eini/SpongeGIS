# ============================================================
# Option A: Calibrated river valley / floodplain model - v5 robust final summary
# ============================================================
# Purpose
...
# Output folder
out_dir <- "X:/download/Mamad/!SpongeGIStest/Kamienna/paper_zone/OptionA_calibrated_valley_floodplain"
...
# They also keep all classes 1, 2, and 3 even if one class is absent.
zone_count_table <- function(zones, valid_zone_values = c(1L, 2L, 3L)) {
  ...
  if (length(zone_vals) == 0) {
    stop("No valid class values 1, 2, or 3 found in the zones raster.")
  }
...
robust_zonal_stat <- function(r, zones, var_name, stat_name) {
  ...
  if (inherits(z, "try-error") || is.null(z) || nrow(z) == 0 || ncol(z) < 2) {
    ...
    out <- data.frame(zone = c(1L, 2L, 3L), tmp = NA_real_, stringsAsFactors = FALSE)
  ...
  names(out)[2] <- out_col
  out[out$zone %in% c(1L, 2L, 3L), , drop = FALSE]
}
...
make_zone_summary_robust <- function(zones, elevation_r, slope_deg_r, slope_pct_r, tpi_large_r, dist_to_river_r) {
  ...
  valid_zone_values <- c(1L, 2L, 3L)
  cell_area_m2_local <- prod(terra::res(zones))
  zone_names <- data.frame(
    zone = valid_zone_values,
    zone_name = c("channel_vector_corridor", "calibrated_valley_floodplain_proxy", "upland"),
    stringsAsFactors = FALSE
  )
...
# -----------------------------
# 14) FINAL 3-CLASS MAP
# -----------------------------
message("Building final 3-class terrain-zone map...")
...
zones_3 <- ifel(channel01 == 1, 1,
                ifel(valley_no_channel, 2,
                     ifel(valid, 3, NA)))
names(zones_3) <- "zone"

writeRaster(channel01, file.path(out_dir, "class1_vector_river_corridor.tif"), overwrite = TRUE, datatype = "INT1U")
writeRaster(ifel(valley_no_channel, 1, 0), file.path(out_dir, "class2_calibrated_valley_no_channel.tif"), overwrite = TRUE, datatype = "INT1U")
writeRaster(zones_3, file.path(out_dir, "zones_3class_vectorRiver_calibratedValley_upland.tif"), overwrite = TRUE, datatype = "INT1U")
...
plot(zones_3, main = "Final zones: 1 river, 2 calibrated valley, 3 upland")
dev.off()
